...
def getUserInfo(user_id):
    ...
    if not os.path.exists(file_path):
        print("<html><body><h1>Erreur: utilisateur non trouvé</h1></body></html>", end="")
        return None
    
    with open(file_path, "r") as file:
        lines = file.readlines()
        if len(lines) < 2:
            print("<html><body><h1>Erreur: données utilisateur incorrectes</h1></body></html>", end="")
            return None
        username = lines[0].strip()
        password = lines[1].strip()
        if len(lines) > 2:
