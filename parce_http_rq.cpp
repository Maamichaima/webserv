...
int check_ispath(std::string str, int &pos, int &flag)
{
	...
	int start = pos + 1;
	// std::cout << str.substr(pos + 1, std::string::npos) << "\n";
	pos = str.substr(pos + 1, std::string::npos).find(' ');
	if(pos == std::string::npos)
		pos = str.size();
	// std::cout  << pos << "\n";
	// std::cout <<"hadi----" <<  str<< "\n";
...
int check_version(std::string str, int &pos, int &flag)
{
	...
	if(size > 5)
		size = 5;
	if(!isMatch(ver.substr(0, size), str.substr(i, size)))
		return 0;
	i += size;
	if(str[i] && !isMatch("\\d+.\\d+", str.substr(i, str.length())))
		return 0;
		pos = i;
	if(str[i] == ' ')
		flag = 3;
...
int main()
{
	...
	std::string htt = "GET /jhbj ";

	// std::string pattern = "[A-Z]+ /\\S* HTTP/\\d+\\.\\d+";
